...
Point Line::intersection(const Line &line, bool *bOnEdge) const
{
    ...
    if (p1 == p2 && line.p1 == line.p2){
        ...
        //std::cout<<"a12 "<<a12[0]<<" "<<a12[1]<<" "<<b12[0]<<" "<<b12[1]<<" "<<ang<<std::endl;
        if (ang < EPS || abs(3.141592653 - ang) < EPS)
            ...
            // 0 < m < 1
            // 0 < n < 1
            //std::cout<<"p1.x  "<<p1.x<<" "<<p1.y<<" "<<line.p1.x<<" "<<line.p1.y<<std::endl;
...
double areaIntersectionEx(const Vertexes &C1, const Vertexes &C2)
{
    ...
    //std::cout<<"findInterPointsEx "<<interVert.size()<<std::endl;
    findInnerPointsEx(C1, C2, innerVert12);
     //std::cout<<"findInnerPointsEx"<<innerVert12.size()<<std::endl;
    findInnerPointsEx(C2, C1, innerVert21);
   // std::cout<<"findInnerPointsEx"<<innerVert21.size()<<std::endl;
    //---------------
